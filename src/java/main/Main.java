...
//               System.out.println(""+);
//                       V_DureeDialogue ki=su.get(0);
//        LocalDateTime date = LocalDateTime.of(2023, Month.MARCH, 1, 12, 0, 0);
        Timestamp tamp = null;
        ...
        int pause=5;
        LocalDateTime resultat = LocalDateTime.of(2023, Month.MARCH, 1, val.getHours(), val.getMinutes(), val.getSeconds());
        resultat = resultat.plusHours(9).plusMinutes(30);
        resultat = resultat.minusMinutes(pause);

        LocalDateTime resultat_prec = LocalDateTime.of(2023, Month.MARCH, 1, val.getHours(), val.getMinutes(), val.getSeconds());
        resultat_prec = resultat.plusHours(8).plusMinutes(0);
        resultat_prec = resultat.minusMinutes(pause);
//        resultat=resultat.plusMinutes(30);
        int cpt = 0;
        ...
        for (int i = 1; i < su.size(); i++) {
            ...
            date = LocalDateTime.of(2023, Month.MARCH, 1, tamp.getHours(), tamp.getMinutes(), tamp.getSeconds());
...

            {
                ...
                if (resultat.minusMinutes(pause).isAfter(LocalDateTime.of(2023, Month.MARCH, 1, 12, 00, 00))) {
                    if (cpt < 1) {
                        resultat = LocalDateTime.of(2023, Month.MARCH, 1, 14, 00, 00);
                        resultat = resultat.minusMinutes(pause);
////
                        resultat = resultat.plusHours(tamp.getHours()).plusMinutes(tamp.getMinutes()).plusSeconds(tamp.getSeconds());
